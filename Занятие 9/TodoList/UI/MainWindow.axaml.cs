    ...
    public partial class MainWindow : Window
    {
        ...
        private async void EditSave_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button btn && btn.DataContext is TodoItemWithEditing item)
            {
                if (item.IsEditing)
                {
                    // Используем UpdateTodoText вместо UpdateTodo
                    await _todoService.UpdateTodoText(item.Todo.Id, item.Todo.Text);
                }
        ...
        private async void CheckBox_Click(object sender, RoutedEventArgs e)
        {
            if (sender is CheckBox checkBox && checkBox.DataContext is TodoItemWithEditing item)
            {
                ...
                // Используем ToggleTodoCompletion вместо UpdateTodo
                await _todoService.ToggleTodoCompletion(item.Todo);
                RefreshItem(item);
            }
        }
